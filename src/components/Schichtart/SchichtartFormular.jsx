...
const SchichtartFormular = ({ schichtart, onReset, darkMode }) => {
  ...
  const isSystemOriginal = isEditing && systemKuerzelSet.has(originalKuerzel?.toUpperCase?.() || '');
  ...
  useEffect(() => {
    ...
    setDauer((endTotal - startTotal) / 60);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEditing && systemKuerzelSet.has((kuerzel || '').toUpperCase())) {
      alert('Das eingegebene Kürzel ist systemreserviert (F, S, N, -, U, KO, KAU) und kann nicht angelegt werden.');
      return;
    }
  ...
  return (
    <div
      className={`p-6 rounded-xl shadow-xl w-full max-w-sm ${
        darkMode ? 'bg-gray-800 text-white border border-gray-700' : 'bg-gray-200 text-gray-900 border border-gray-300'
      }`}
    >
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        <input
          ...
          maxLength={3}
          disabled={isSystemOriginal /* Nur bei Bearbeitung eines System-Kürzels sperren */}
        ...
        <label className="flex gap-2 items-center">
          <input
            ...
            // NICHT sperren: Arbeitszeit darf bei System-Kürzeln geändert werden
          />
        ...
        {!ignoriertArbeitszeit && (
          <>
            ...
            {end < start && <div className="text-yellow-400 text-sm font-semibold">Endet am nächsten Tag</div>}
        ...
        <label className="flex gap-2 items-center">
          <input
            ...
            disabled={isSystemOriginal /* System-Kürzel: kein Toggle von Soll-Plan relevant */}
          />
          Soll-Plan relevant?
        </label>
        ...
        <input
          ...
          disabled={isSystemOriginal /* System-Kürzel: Bezeichnung gesperrt */}
        ...
        {istSuperAdmin && (
          <>
            <select
              ...
              disabled={isSystemOriginal /* System-Kürzel: Stamm-Zuordnung gesperrt */}
        ...
        {isSystemOriginal && (
          ...
          >
            ⚠️ <strong>Hinweis:</strong> Dieses Kürzel ist systemrelevant – nur <strong>Farbe</strong> und{' '}
            <strong>Arbeitszeit</strong> dürfen geändert werden.
          </div>
        )}

        <div className="flex justify-between mt-1">
          <button type="submit" className="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded font-medium">
      ...
      {infoOffen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center backdrop-blur-sm z-50">
          <div className="bg-white dark:bg-gray-900 text-black dark:text-white p-6 rounded-xl shadow-xl w-[90%] max-w-lg animate-fade-in relative">
            <button className="absolute top-2 right-2 text-gray-400 hover:text-red-500" onClick={() => setInfoOffen(false)}>
              <X size={20} />
            </button>
